...
// Ensure $conn from config is available
global $conn;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'add') {
    ...
    if (safeQuery($sql, [$lrn, $rfid_uid, $fullname, $gender, $birth_date, $parent_name, $parent_contact, $address, $section_id, $enrolled_date, null, $photo], 'ssssssssisss')) {
        ...
        if ($create_account && $email) {
            ...
            if (empty($final_username)) {
                ...
                $counter = 1;
                while (mysqli_num_rows(safeQuery("SELECT id FROM users WHERE username = ?", [$final_username], 's')) > 0) {
            ...
            $sql_account = "INSERT INTO users (username, password, role, email, phone, fullname, status, email_verified, registration_code, created_at) 
                           VALUES (?, ?, 'student', ?, ?, ?, 'active', 1, 'teacher_created', NOW())";
            if (safeQuery($sql_account, [$final_username, $hashed_password, $email, $phone, $fullname], 'sssss')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update') {
    ...
    if (safeQuery($sql, [$fullname, $gender, $birth_date, $parent_name, $parent_contact, $address, $section_id, $photo, $status, $id], 'ssssssissi')) {
...
if (isset($_GET['delete'])) {
    ...
    // Fetch student photo before deleting (no user_id column in students table)
    $student = safeQuery("SELECT photo FROM students WHERE id = ?", [$id], 'i');
...
if ($search) {
    $sql .= " AND (s.fullname LIKE ? OR s.lrn LIKE ? OR s.rfid_uid LIKE ?)";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $params[] = "%$search%";
    $types .= "sss";
...
$students = safeQuery($sql, $params, $types);
...
<body>
    ...
    <div class="main-content">
        <div class="students-container">
            ...
            <?php if ($error_msg): ?>
                <div class="toast-notification toast-error" id="errorToast"><i class="fas fa-exclamation-circle"></i>
                    <?php echo ucfirst(str_replace('_', ' ', $error_msg)); ?></div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="table-wrapper">
                        <table id="studentsTable">
                            ...
                            <td><?php echo htmlspecialchars($student['fullname']); ?>
                        </div>
                        <td><?php echo htmlspecialchars($student['lrn']); ?>
                    </div>
                    <td>
                ...
                <td><?php echo htmlspecialchars($student['level_name'] . ' - ' . $student['section_name']); ?>
            </div>
            <td><?php echo htmlspecialchars($student['gender']); ?></div>
            <td><?php echo htmlspecialchars($student['parent_name']); ?></div>
            <td><?php echo htmlspecialchars($student['parent_contact']); ?></div>
            <td><span
                    class="badge badge-<?php echo $student['status'] == 'active' ? 'success' : 'danger'; ?>"><?php echo ucfirst($student['status']); ?></span>
                </div>
            <td>
                <div class="action-buttons">
                    ...
                    <button class="btn-icon" onclick="editStudentModal(<?php echo $student['id']; ?>)" title="Edit Student"><i
                    ...
                    <form id="addStudentForm" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid-dashboard" style="gap: 12px;">
                            ...
                            <div class="form-group"><label class="form-label">Section <span
                                ...
                                <select name="section_id" class="form-control" required>
                                    ...
                                    <?php $secs = safeQuery("SELECT s.*, yl.level_name FROM sections s JOIN year_levels yl ON s.year_level_id = yl.id WHERE s.teacher_id = ? AND s.status = 'active' ORDER BY yl.sort_order", [$teacher_id], 'i');
                                    while ($sec = mysqli_fetch_assoc($secs)): ?>
                                        <option value="<?php echo $sec['id']; ?>">
                                            <?php echo $sec['level_name'] . ' - ' . $sec['section_name']; ?></option>
                                    <?php endwhile; ?>
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            <label style="display: flex; align-items: center; gap: 8px;">
                                <input type="checkbox" name="create_account" value="1"> Create student login account
                            </label>
                        </div>
        ...
        <script>
            let deleteStudentId = null, currentStudentId = null;
            ...
            function regenerateQR() { if (currentStudentId && confirm('Regenerate QR code for this student?')) window.location.search = `?regenerate_qr=${currentStudentId}`; }
                ...
                fetch(`api/get_student.php?id=${id}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            const sections = <?php
                            $sections_json = [];
                            $secs = safeQuery("SELECT s.*, yl.level_name FROM sections s JOIN year_levels yl ON s.year_level_id = yl.id WHERE s.teacher_id = ? AND s.status = 'active' ORDER BY yl.sort_order", [$teacher_id], 'i');
                            while ($sec = mysqli_fetch_assoc($secs))
                                $sections_json[] = $sec;
                            ...
                            let userData = {};
                            if (data.user_id) {
                                const userFetch = fetch(`api/get_user.php?id=${data.user_id}`).then(res => res.json()).catch(() => ({}));
                            }
                        ...
                        } else { alert('Failed to load student details'); }
