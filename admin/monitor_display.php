...
// Get statistics
$time_in_count = mysqli_fetch_assoc(safeQuery("SELECT COUNT(*) as count FROM attendance_log WHERE log_date = CURDATE() AND log_type IN ('morning_in', 'afternoon_in')"))['count'];
$time_out_count = mysqli_fetch_assoc(safeQuery("SELECT COUNT(*) as count FROM attendance_log WHERE log_date = CURDATE() AND log_type IN ('morning_out', 'afternoon_out')"))['count'];
$students_today = mysqli_fetch_assoc(safeQuery("SELECT COUNT(DISTINCT student_id) as count FROM attendance_log WHERE log_date = CURDATE()"))['count'];
$total_enrolled = mysqli_fetch_assoc(safeQuery("SELECT COUNT(*) as count FROM students WHERE status = 'active'"))['count'];
...
$recent_logs = safeQuery($recent_sql, [], '');
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Monitor Display - Admin</title>
...
<body>
    ...
    <script>
        ...
        function loadLatestStudentDetails() {
            $.ajax({
                url: '../api/get_latest_student.php',
                method: 'GET',
                dataType: 'json',
            ...
            let html = `
                <div class="student-detail-card">
                    ...
                    <div class="detail-row">
                        ...
                        <div class="detail-value">${escapeHtml(student.teacher_name) || 'Not assigned'}</div>
                    </div>
                    <div class="detail-row">
        ...
        function loadCurrentStatus() {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.length > 0) {
                        data.forEach(student => {
                            let photoHtml = '';
                            if (student.photo && student.photo !== 'default_student.png') {
                                photoHtml = `<img src="../uploads/students/${student.photo}" style="width: 35px; height: 35px; border-radius: 50%; object-fit: cover;">`;
                            } else {
                                photoHtml = `<div style="width: 35px; height: 35px; background: var(--pink-light); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-user" style="font-size: 14px; color: var(--pink);"></i>
                                </div>`;
                            }
                            ...
                            html += `<tr>
                                ...
                                <td>${student.morning_in || '-'}</td>
                                <td>${student.morning_out || '-'}</td>
                                <td>${student.afternoon_in || '-'}</td>
                                <td>${student.afternoon_out || '-'}</td>
                                <td><span class="badge badge-${student.present ? 'success' : 'danger'}">${student.present ? 'Present' : 'Absent'}</span></td>
                            </tr>`;
                        });
                    } else {
                        html = `<tr><td colspan="8" class="text-center" style="padding: 40px; color: #999;">
                ...
                error: function () {
                    document.getElementById('statusTableBody').innerHTML = '<tr><td colspan="8" class="text-center" style="padding: 40px;">Failed to load status</td></tr>';
                }
            });
        }
        ...
        function loadRecentActivity() {
            $.ajax({
                url: '../api/get_recent_activity.php',
                method: 'GET',
                success: function (data) {
                    if (data) {
